...
template<class T>
inline static std::string MatrixSplice2String(const T &mat, const std::size_t size, const std::string valueSep, const std::string tab = {}){
    ...
    for(std::size_t i = 0;i < outsize;i ++){
       ret += tab + std::to_string(mat[i]) + valueSep;
    }
    
    if(size > OUT_STREAM_MAX_ELEMENT){
        ret += tab + OUT_STREAM_LOOP_CHAR + OUT_STREAM_COMET_CHAR + enter;
    }
    return ret + tab + std::to_string(mat[size - 1]) + enter + OUT_STREAM_RIGHT_SEP2;
}

namespace std{
std::string to_string(const Matrix1D &mat){
    return MatrixSplice2String(mat, mat.d1, OUT_STREAM_COMET_CHAR, OUT_STREAM_SPACE_CHAR);
}

std::string to_string(const Matrix2D &mat){
    return MatrixSplice2String(mat, mat.d2, std::string(OUT_STREAM_COMET_CHAR) + OUT_STREAM_ENTER_CHAR, OUT_STREAM_TAB_CHAR
                               );
}

std::string to_string(const Matrix3D &mat){
    return MatrixSplice2String(mat, mat.d3, std::string(OUT_STREAM_COMET_CHAR) + OUT_STREAM_ENTER_CHAR, std::string(OUT_STREAM_TAB_CHAR));
}

std::string to_string(const Matrix4D &mat){
    return MatrixSplice2String(mat, mat.d4, std::string(OUT_STREAM_COMET_CHAR) + OUT_STREAM_ENTER_CHAR, std::string(OUT_STREAM_TAB_CHAR));
}
